...
def infer_attentivefp(member, smiles_list, labels, task_config, device, batch_size=64):
    ...
    for i, smiles in enumerate(smiles_list):
        data = smiles_to_pyg_data(smiles, label=labels[i])
        if data is not None:
            dataset.append(data)
            valid_idxs.append(i)
    ...
    for batch in loader:
        ...
        for k in range(bs):
            all_logits[valid_idxs[ptr + k]] = out_np[k]
        ptr += bs
...
def infer_pretrained_gin(member, smiles_list, labels, task_config, device, batch_size=128):
    ...
    # Track original indices to handle skipped molecules safely.
...
def infer_xgb(member, smiles_list, labels, task_config, **_):
    ...
    for smi in smiles_list:
        ...
        if mol is None:
            ...
            arr = np.zeros((member["nbits"],), dtype=np.float32)
