...
Weights are parameterized via softmax so the optimization is unconstrained:
...
def infer_graph_model(member, smiles_list, labels, task_config, device, batch_size=64):
    ...
    for batch in loader:
        ...
        for k in range(bs):
            all_logits[valid_idxs[ptr + k]] = logits_np[k]
        ptr += bs
...
def main():
    ...
    # -- Optimize per-task weights on val ----------------------------------
    ...
    print(f"  {'Task':<22} {'Simple':>8} {'Weighted':>9} {'Delta':>7}")
    print("  " + "-" * 48)
    for task in task_config.task_names:
        ...
        delta = w - s
        flag  = "^" if delta > 0.002 else ("v" if delta < -0.002 else " ")
