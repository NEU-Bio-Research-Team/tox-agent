...
Example: TOXAGENT_OPENCODE_MODEL=openai/gpt-5.6-luna ./devops/scripts/legacy/run_local_phase3.sh
EOF
  exit 2
fi
...
start_in_dir predictor "$REPO_ROOT" env PYTHONPATH="$REPO_ROOT/backend/predictor/src" "$PYTHON_BIN" -m uvicorn toxpred.api.app:app --host 127.0.0.1 --port 8080
...
start_in_dir ocr "$REPO_ROOT" env PYTHONPATH="$REPO_ROOT/backend/ocr/src" "$OCR_PYTHON" -m uvicorn toxocr.api.app:app --host 127.0.0.1 --port 8090
OCR_PID=$LAST_PID
...
Run ./devops/scripts/legacy/smoke_local_phase3.sh in a second terminal to execute an
