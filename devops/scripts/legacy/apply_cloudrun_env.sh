...
ENV_FILE="${1:-$REPO_ROOT/backend/predictor/deploy/cloudrun-env.yaml}"

if [[ ! -f "$ENV_FILE" ]]; then
  echo "Env file not found: $ENV_FILE"
  echo "Create it from backend/predictor/deploy/cloudrun-env.yaml and paste your keys first."
  exit 1
fi
