...
from research.legacy_backend.data import load_clintox
from research.legacy_backend.train import evaluate_model
from research.legacy_backend.pipelines import load_mlp_model, load_gnn_model
from research.legacy_backend.utils import set_seed
...
try:
    ...
    from research.legacy_backend.graph_data import smiles_list_to_pyg_dataset
    from research.legacy_backend.graph_train import evaluate_model as evaluate_graph_model
    PYG_AVAILABLE = True
except ImportError:
    PYG_AVAILABLE = False
    ...
    if model_name == 'Baseline MLP':
        ...
        try:
            ...
            if not model_path.exists():
                return None, None
            from research.legacy_backend.utils import get_default_config
            config = get_default_config()
            model = load_mlp_model(model_path, config)
            from research.legacy_backend.featurization import featurize_batch
        ...
        try:
            ...
            from research.legacy_backend.train import predict_with_torch_molecule_model
        ...
        # Load DeepChem DMPNN
        try:
            models_dir = project_root / ".data" / "models" / "dmpnn_model"
            import pickle
        ...
        try:
            if 'GATv2' in model_name:
                model_path = project_root / ".data" / "models" / "gatv2_model" / "best_model.pt"
                from research.legacy_backend.graph_models import GATv2MolecularPredictor, create_gatv2_model
                from research.legacy_backend.graph_data import get_feature_dims
                ...
                model_path = project_root / ".data" / "models" / "gin_model" / "best_model.pt"
                from research.legacy_backend.graph_models_gin import GINMolecularPredictor, create_gin_model
                from research.legacy_backend.graph_data import get_feature_dims
                num_node_features, num_edge_features = get_feature_dims()
                ...
                model_path = project_root / ".data" / "models" / "smilesgnn_model" / "best_model.pt"
                from research.legacy_backend.graph_models_hybrid import create_hybrid_model
                from research.legacy_backend.graph_data import get_feature_dims
                from research.legacy_backend.smiles_tokenizer import create_tokenizer_from_smiles
                import pickle

                num_node_features, num_edge_features = get_feature_dims()

                # Load tokenizer
                tokenizer_path = project_root / ".data" / "models" / "smilesgnn_model" / "tokenizer.pkl"
