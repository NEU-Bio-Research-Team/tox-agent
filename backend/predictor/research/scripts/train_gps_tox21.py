#!/usr/bin/env python3
"""
Train GPS Graph Transformer on Tox21 multi-task toxicity benchmark.

Usage:
    conda activate drug-tox-env
    python scripts/train_gps_tox21.py \
        --config backend/predictor/configs/tox21_gps_config.yaml --device cuda
"""
...
from research.legacy_backend.graph_data import smiles_to_pyg_data, get_feature_dims
from research.legacy_backend.gps_model import create_gps_model
from research.legacy_backend.graph_train import evaluate_multitask_model, MaskedFocalLoss
from research.legacy_backend.utils import ensure_dir, save_metrics, set_seed
...
def main():
    ...
    parser.add_argument("--config", type=str, default="backend/predictor/configs/tox21_gps_config.yaml")
    ...
    print("=" * 70)
    print("GPS Graph Transformer — Tox21")
    print("=" * 70)
    ...
    model = GraphModelWrapper(gps).to(device)
    ...
    out_dir = project_root / oc.get("model_dir", ".data/models/tox21_gps_model")
    ensure_dir(str(out_dir))
    torch.save(gps.state_dict(), out_dir / "best_model.pt")
    ...
    print(f"\nTest AUC-ROC: {flat['test_mean_auc_roc']:.4f}")
